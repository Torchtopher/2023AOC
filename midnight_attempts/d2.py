...
for y, line in enumerate(s):
    # find every number
    num_split = re.findall(r'\d+|[a-zA-Z]+', line)
    print(num_split)
    print(line)
    
    # make dict of all numbers that occur more than once
    occurance_dict = {}
    for num in num_split:
        if num_split.count(num) > 2:
            print("BAD SHIT")
            exit()
        occurance_dict[num] = 0
    print(num_split)
    for num_split_idx, val in enumerate(num_split):
        # make dict of all numbers that occur more than once
        occurance_dict[val] += 1
        if occurance_dict[val] == 1:    
            x = line.index(val)
        else:
            x = line.rindex(val)
            print(f"Using rindex of {x}")

        done = False
        for digit in val:
            # check in a 3x3 grid around the number
            for i in range(-1, 2):
                for j in range(-1, 2):
                    if i == 0 and j == 0:
                        continue
                    if x + j < 0 or x + j >= len(line):
                        continue
                    if y + i < 0 or y + i >= len(s):
                        continue
                    print(val)
                    print(y, i, x, j)
                    if s[y+i][x+j] != '.' and not s[y+i][x+j].isdigit():
                        print(f'Found {val} at {y}, {x}')
                        ans += int(val)
                        done = True
                        break
                if done:
...
print(ans)
exit()
# find all non . non numeric characters
for y, line in enumerate(s):
    for x, val in enumerate(line):
        if val != "*":
            continue
        would_add = []
        print(f"On char {val} at {y}, {x}")
        # check in a 3x3 grid around the number
        done = False
        ...
        for i in range(-1, 2):
            ...
            for j in range(-1, 2):
                ...
                if y + i < 0 or y + i >= len(s):
                    continue
                if s[y+i][x+j].isdigit():
                    number_string = s[y+i][x+j]
                    # forward search
                    for k in range(x+j+1, len(line)):
                        if s[y+i][k].isdigit():
                            number_string += s[y+i][k]
                        else:
                            break
                    # backward search
                    for k in range(x+j-1, -1, -1):
                        if s[y+i][k].isdigit():
                            print(line[k])
                            number_string = s[y+i][k] + number_string
                        else:
                            break
                    
                    print(f'Found {number_string} at {y}, {x}')
                    if number_string in number_list:
                        continue
                    would_add.append(int(number_string))
                    number_list.append(number_string)
                    done = True
        if len(would_add) == 2:
            ans += would_add[0] * would_add[1]
